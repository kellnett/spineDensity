...
# I. Intro
  ...
  # A. 
    # 1. This code models Yadav et al. 2012 analysis of dendritic spines using unsupervised agglomerative hierarchical clustering method
    # 2. Initial creation of this code was by S. Johnson and was tailored towards clustering of attachment points
      # 3. Recent updates have been made by K. Nett in collaboration with J. Kent to 
          # a. Improve calculations/readability
          # b. Establish clustering method that looks at spine heads using 3D coordinates
  
  # B. Goals of code:
    # 1. Summarise output data from NeuronStudio
    # 2. Calculate clustering of spines using different methods
      # a. Method coded by S.Johnson looking at 1D attachment points (i.e. SOMA.DISTANCE) using the 'agnes' function from package 'cluster'
      # b. Method still utilizing 1D attachment points (i.e. SOMA.DISTANCE) but using function 'IdClusters' from the package 'DECIPHER'
      # c. Method looking at clustering of 3D spine head locations using function 'IdClusters' from the package 'DECIPHER'
      # d. Nearest neighbor analysis using both 1D attachment points and 3D spine head locations
 
  # C. Packages used in code
    # 1. Installation Instructions (note: you only need to install these once, then library references are sufficient)  
      # a. enter    install.packages("package name")     into console of Rstudio, i.e. install.packages("dplyr")
      # b. special cases
        # i. package "bio3d", enter   install.packages("bio3d", dependencies = TRUE)
        # ii. package "DECIPHER", enter{     source("https://bioconductor.org/biocLite.R")
        #                                    biocLite("DECIPHER")
        #                                                         }
        # when prompted, enter 'a' for "all"

    # 2. Library references for installed packages 
          library(dplyr)           
          library(ggplot2)        
          library(lattice)
...
# II. Initiallizing Code
# A. Select file(s) and define group
  ...
  n          <- readline(prompt = "Enter group name:")  # prompt given in console to enter the animal treatment group (i.e. Sal-D0)

# II. File list for-loop: Apply code to each file in define file list (above)
...
## length when there is more than 1 section
    ...
    density_mushroom <- sum(as.numeric(df$TYPE == "mushroom"), na.rm = TRUE)/total_length  # calculate mushroom spine density
    density_thin     <- sum(as.numeric(df$TYPE == "thin"), na.rm = TRUE)    /total_length  # calculate thin spine density
    density_stubby   <- sum(as.numeric(df$TYPE == "stubby"), na.rm = TRUE)  /total_length  # calculate stubby spine density
...
## the original nearest neighbor analysis using euclidean distance of attachment points
    ...
    for(k in 1:10) {  # random ac for-loop, use random distances from soma to generate list of randomized ac values
...
UPGMA_obsv_1D    <- IdClusters(dist_1D, method = "UPGMA", cutoff = 0.75, showPlot = FALSE)  #runs cluster analysis with cutoff from Yadav paper on observed spines
...
num_clusters_1D     <- sum(cluster_freq_1D$is_clustered, na.rm = TRUE) #count how many clusters are on this segment
spines_clustered_1D <- sum(cluster_freq_1D$Freq > 1, na.rm = TRUE)
...
test_spines_clustered_all_1D <- colnames("test spines clustered 1D")
test_num_clusters_all_1D <- data.frame()
test_num_clusters_all_1D <- colnames("test number of clusters 1D")
...
# 1D random spines for-loop
  for(j in 1:10){
    ...
    num_clusters_test_1D <- sum(cluster_freq_test_1D$is_clustered, na.rm = TRUE) # add how many 1s (or how many clusters have >1 spines) to get true number of clusters
    num_clusters_test_1D[is.na(num_clusters_test_1D)] <- 0 #changes possible NA from 0 clusters to 0
    num_clusters_test_1D <- as.numeric(num_clusters_test_1D)
    ...
    spines_clustered_test_1D <- sum(cluster_freq_test_1D$Freq > 1, na.rm = TRUE)
    ...
    spines_clustered_test_1D <- data.frame(spines_clustered_test_1D)
...
data_all$animal_num <- lapply(data_all$animal_num, 
                              function(x) unlist(strsplit(x, "L"))[1])
data_all$animal_num <- lapply(data_all$animal_num, 
                              function(x) unlist(strsplit(x, "N"))[1])

data_all_by_file$animal_num <- lapply(data_all_by_file$file,
                                      function(x) unlist(strsplit(x, "-"))[2])
...
dir.create("analysis3")  #creates a directory to create a file on the computer
savePath <- paste(filePath, "/", "analysis3", collapse = "/", sep = "") # creates the path where files can be saved
...
write.csv(animal_avg, "animal_avg.csv")
