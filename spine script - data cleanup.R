...
for(i in 1:length(fileList))  {                             # peform code inside {} for each .csv file in the file list
  ...
  UPGMA_obsv_3D <- IdClusters(dist_3D, method = "UPGMA", cutoff = 0.75, showPlot = FALSE) #run cluster analysis with cutoff used in Yadav paper, gives cluster number associated with spine  UPGMA_obsv_3D$rn <- rownames(UPGMA_obsv_3D)   #adds a column with rownows to keep spine ID, not sure if this step is neccessary
...
#For the following comments 1-6 we are applying the same code to two separate data-frames of different lengths
...
#6) Change animal_num and dendrite location(PDB) to a vector rather than a list, which is important for executing upcoming code
...
location_avg_file <- data_all_by_file %>% 
  ...
  summarise(Cscore_1D           = mean(Cscore_1D),
            cScore_ac_1D        = mean(cScore_ac_1D),
            dendrite_ac         = mean(dendrite_ac),
            num_clusters_1D     = mean(num_clusters_1D),
            spines_clustered_1D = mean(spines_clustered_1D),
            spines_not_1D       = mean(spines_not_1D),
            num_clusters_3D     = mean(num_clusters_3D),
            spines_clustered_3D = mean(spines_clustered_3D),
            spines_not_3D       = mean(spines_not_3D),
            #       Cscore_3D           = mean(Cscore_3D),
...
dir.create("5-31-19")  #creates a directory to create a file on the computer
savePath <- paste(filePath, "/", "5-31-19", collapse = "/", sep = "") # creates the path where files can be saved
